        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <select name="product_id" id="product-select" class="w-auto h-10 dark:bg-gray-700" required>
                ...
                @foreach($products as $product)
                    <option value="{{ $product->id }}">{{ $product->description }} {{ $product->brand }}</option>
                @endforeach
            </select>

            <div>
                <label for="quantity" class="text-sm sm:text-base md:text-lg lg:text-xl font-bold">Quantidade</label>
                <input type="text" name="quantity" id="quantity" class="w-full h-10 dark:bg-gray-700 text-right" required>
            </div>
            <div>
                <label for="price" class="text-sm sm:text-base md:text-lg lg:text-xl font-bold">Preço</label>
                <input type="text" name="price" id="price" class="w-full h-10 dark:bg-gray-700 text-right" required>
            </div>
